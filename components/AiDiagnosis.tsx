  ...
  return (
    <>
      ...
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl font-bold mb-6 text-white">Diagnóstico de Plantas con IA</h1>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <Card>
            <h2 className="text-xl font-semibold mb-4 text-emerald-500">1. Subir o Tomar Foto</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-400 mb-2">Opción 1: Subir Archivo</label>
                <input
                  type="file"
                  accept="image/*"
               ...
               <div>
                  <label className="block text-sm font-medium text-gray-400 mb-2">Opción 2: Usar Cámara</label>
                  <button onClick={() => setIsCameraOpen(true)} className="w-full py-2 px-4 bg-gray-600 hover:bg-gray-700 text-white font-bold rounded-md transition-colors">
                      Tomar Foto
                  </button>
              </div>
              {imagePreview && (
              ...
              <div>
                <label htmlFor="notes" className="block text-sm font-medium text-gray-400 mb-2">Notas Adicionales</label>
                <textarea
                  id="notes"
                  rows={4}
                  value={notes}
              ...
              <button
                ...
                className="w-full py-2 px-4 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-md transition-colors disabled:bg-gray-500 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
          ...
          <Card>
            <h2 className="text-xl font-semibold mb-4 text-emerald-500">2. Resultado del Diagnóstico</h2>
            {isLoading && <div className="flex justify-center items-center h-full"><Spinner /></div>}
            {error && <p className="text-red-500 text-center">{error}</p>}
            {result && (
              <div className="space-y-4 text-gray-300">
                <div>
                  <h3 className="font-bold text-lg text-white">Salud General</h3>
                  <p>{result.overall_health_assessment}</p>
                </div>
                <div>
                  <h3 className="font-bold text-lg text-white">Diagnóstico</h3>
                  <p>{result.diagnosis}</p>
                </div>
                <div>
                  <h3 className="font-bold text-lg text-white">Consejos Preventivos</h3>
            ...
            {!isLoading && !result && !error && (
              <div className="flex justify-center items-center h-full">
                <p className="text-gray-500">Tu diagnóstico aparecerá aquí.</p>
              </div>
            )}
          </Card>
        </div>
