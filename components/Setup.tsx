...
const BatchSetup: React.FC = () => {
    ...
    const { saveCrop, setActiveCropId } = useCrops();
    ...
    const getRoomsForLocation = (parentId: string) => locations.filter(l => l.parentId === parentId);

    const totalPlantsSelected = useMemo(() => {
        return Object.values(selectedPlants).reduce((sum, count) => sum + (count || 0), 0);
    }, [selectedPlants]);
    
    const mainGeneticsId = useMemo(() => {
        const firstSelectedBatchId = Object.keys(selectedPlants).find(id => selectedPlants[id] > 0);
    ...
    return (
        <form className="space-y-8" onSubmit={handleSubmit}>
            <div>
                <h2 className="text-xl font-semibold text-emerald-500 pb-2 border-b border-gray-700 mb-4">1. Seleccionar Plantas de Lotes Disponibles</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {availableBatches.length > 0 ? availableBatches.map(batch => {
                        const genetic = genetics.find(g => g.id === batch.geneticsId);
                        return (
                            <div key={batch.id} className="p-4 rounded-lg bg-gray-700 border border-gray-600">
                                <p className="font-bold text-white">{batch.id}</p>
                                <p className="text-sm text-gray-300">{genetic?.name}</p>
                                <p className="text-xs text-gray-400">Disponibles: {batch.availablePlantCount}</p>
                                <div className="mt-2">
                                    <label htmlFor={`plants-${batch.id}`} className="block text-sm font-medium text-gray-400 mb-1">Cantidad a usar:</label>
                                    <input
                                        id={`plants-${batch.id}`}
                                        type="number"
                                        min="0"
                                        ...
                                        onChange={e => handlePlantSelectionChange(batch.id, parseInt(e.target.value, 10))}
                                        className="w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded-md"
                                    />
                                </div>
                            </div>
                        );
                    }) : <p className="text-gray-500 col-span-full">No hay lotes en "Pre-vegetación" con plantas disponibles. Avanza un lote desde "Gestión de Lotes".</p>}
                </div>
            </div>
            <div>
                <h2 className="text-xl font-semibold text-emerald-500 pb-2 border-b border-gray-700 mb-4">2. Detalles del Cultivo</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label htmlFor="location" className="block text-sm font-medium text-gray-400 mb-2">Ubicación (será el nombre del cultivo)</label>
                        <select id="location" value={locationId} onChange={e => setLocationId(e.target.value)} className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md" required>
                    ...
                    <div>
                        <label htmlFor="startDate" className="block text-sm font-medium text-gray-400 mb-2">Fecha de Inicio (Paso a Vegetación)</label>
                        <input id="startDate" type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md" required />
                    </div>
                </div>
            </div>
            <div className="flex justify-between items-center pt-4 border-t border-gray-700">
                <div className="text-lg font-bold text-white">
                    Total de Plantas Seleccionadas: <span className="text-emerald-500">{totalPlantsSelected}</span>
                </div>
                <button type="submit" className="py-2 px-6 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-md transition-colors disabled:opacity-50" disabled={totalPlantsSelected === 0}>
                    Iniciar Cultivo
                </button>
            </div>
        </form>
...
const ManualSetup: React.FC = () => {
    ...
    return (
         <form className="space-y-6" onSubmit={handleManualSubmit}>
            <div >
                <label className="block text-sm font-medium text-gray-400 mb-2">Ubicación (será el nombre del cultivo)</label>
                <select name="locationId" value={cropDetails.locationId} onChange={handleDetailChange} className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md" required>
             ...
             <div className="p-4 bg-gray-900/50 rounded-lg">
                <h3 className="text-lg font-semibold text-emerald-500 mb-4">Genéticas y Cantidad de Plantas</h3>
                <div className="space-y-3">
                    {geneticsSelection.map((sel, index) => (
                        <div key={sel.id} className="grid grid-cols-1 sm:grid-cols-[1fr_auto_auto] gap-3 items-center p-2 bg-gray-700/50 rounded-md">
                            <select 
                                value={sel.geneticId} 
                            ...
                            <input 
                                ...
                                className="w-full sm:w-32 px-3 py-2 bg-gray-700 border border-gray-600 rounded-md" 
                                required 
                            />
                            <button 
                                type="button"
                ...
                <button type="button" onClick={addGeneticRow} className="mt-4 py-2 px-4 bg-gray-600 hover:bg-gray-500 text-white font-bold rounded-md text-sm">
                    + Añadir Genética
                </button>
            </div>

            <div className="p-4 bg-gray-900/50 rounded-lg">
                <h3 className="text-lg font-semibold text-emerald-500 mb-4">Fechas de Inicio de Etapas</h3>
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
                    <div>
                        <label className="block text-sm font-medium text-gray-400 mb-2">Fecha Clonación/Siembra *</label>
                        <input name="cloningDate" type="date" value={cropDetails.cloningDate} onChange={handleDetailChange} className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md" required />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-400 mb-2">Paso a Vegetación (opcional)</label>
                        <input name="vegDate" type="date" value={cropDetails.vegDate} onChange={handleDetailChange} className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-400 mb-2">Paso a Floración (opcional)</label>
                        <input name="flowerDate" type="date" value={cropDetails.flowerDate} onChange={handleDetailChange} className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md" />
                    </div>
                 </div>
            </div>
             <div className="flex justify-between items-center pt-4 border-t border-gray-700">
                 <div className="text-lg font-bold text-white">
                    Total de Plantas: <span className="text-emerald-500">{totalPlants}</span>
                </div>
...
const Setup: React.FC = () => {
    ...
    return (
        <div>
            <h1 className="text-3xl font-bold mb-6 text-white">Configurar Nuevo Cultivo</h1>
            <Card>
                 {isAdmin ? (
                    <div className="mb-6 border-b border-gray-700">
                        <nav className="-mb-px flex space-x-8" aria-label="Tabs">
                            <button
                                ...
                                className={`${ setupMode === 'batch' ? 'border-emerald-500 text-emerald-400' : 'border-transparent text-gray-400 hover:text-gray-200 hover:border-gray-500'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm focus:outline-none`}
                            >
                                Desde Lote (Para cultivos nuevos)
                            </button>
                            <button
                                onClick={() => setSetupMode('manual')}
                                className={`${ setupMode === 'manual' ? 'border-emerald-500 text-emerald-400' : 'border-transparent text-gray-400 hover:text-gray-200 hover:border-gray-500'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm focus:outline-none`}
                            >
