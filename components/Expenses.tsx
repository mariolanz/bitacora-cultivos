...
const Expenses: React.FC = () => {
    ...
    const visibleExpenses = useMemo(() => {
        ...
        return expenses.filter(e => e.locationId === currentUser?.locationId);
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <Card>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                         ...
                         <div>
                            <label htmlFor="category" className="block text-sm text-gray-400">Categoría</label>
                            <select id="category" name="category" value={form.category} onChange={handleInputChange} className="w-full mt-1 px-3 py-2 bg-gray-700 border border-gray-600 rounded-md" required>
                        ...
                        <div>
                            <label htmlFor="amount" className="block text-sm text-gray-400">Monto ($)</label>
                            <input type="number" step="0.01" id="amount" name="amount" value={form.amount} onChange={handleInputChange} placeholder="0.00" className="w-full mt-1 px-3 py-2 bg-gray-700 border border-gray-600 rounded-md" required />
                        </div>
                        <div>
                        ...
                        {currentUser?.roles.includes(UserRole.ADMIN) && (
                            <div>
                                ...
                                <select id="locationId" name="locationId" value={form.locationId} onChange={handleInputChange} className="w-full mt-1 px-3 py-2 bg-gray-700 border border-gray-600 rounded-md" required>
                        ...
                        <div className="flex gap-2 pt-2">
                             {editingExpense && <button type="button" onClick={handleCancelEdit} className="w-full py-2 px-4 bg-gray-600 hover:bg-gray-700 text-white font-bold rounded-md">Cancelar</button>}
                            <button type="submit" className="w-full py-2 px-4 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-md">
            ...
            <div className="lg:col-span-2">
                <Card>
                    <h2 className="text-xl font-semibold text-emerald-500 mb-4">Historial de Gastos</h2>
                        ...
                        <table className="w-full text-sm text-left text-gray-400">
                            ...
                            <tbody>
                                {visibleExpenses.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(exp => (
                                    <tr key={exp.id} className="bg-gray-800 border-b border-gray-700 hover:bg-gray-600">
