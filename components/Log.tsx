...
const DynamicNumberInput: React.FC<{
  ...
  const handleDecrement = () => {
    const newValue = parseFloat(Math.max(min, value - step).toFixed(precision));
    onChange(newValue);
  };

  const handleIncrement = () => {
    const newValue = parseFloat(Math.min(max, value + step).toFixed(precision));
    onChange(newValue);
  };
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2">
        <button type="button" onClick={handleDecrement} className="p-1 rounded-full bg-gray-600 hover:bg-gray-500 text-white w-8 h-8 flex items-center justify-center font-bold text-lg flex-shrink-0" aria-label={`Decrease ${label}`}>-</button>
        <input
          type="range"
          min={min}
          max={max}
        ...
        />
        <button type="button" onClick={handleIncrement} className="p-1 rounded-full bg-gray-600 hover:bg-gray-500 text-white w-8 h-8 flex items-center justify-center font-bold text-lg flex-shrink-0" aria-label={`Increase ${label}`}>+</button>
        <input
          type="number"
          step={step}
          min={min}
