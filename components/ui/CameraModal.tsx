...
const CameraModal: React.FC<{
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-85 flex items-center justify-center z-50 p-2">
      <div className="bg-gray-800 p-4 rounded-lg max-w-4xl w-full flex flex-col max-h-full">
        ...
        <div className="flex justify-center gap-4 flex-shrink-0">
          <button onClick={handleClose} className="px-6 py-2 rounded bg-gray-600 hover:bg-gray-700 text-white font-semibold">Cerrar</button>
          <button onClick={handleCapture} className="px-6 py-2 rounded bg-emerald-600 hover:bg-emerald-700 text-white font-semibold" disabled={!!error}>Capturar Foto</button>
        </div>
      </div>
    </div>
  );
