...
// FIX: Add Equipment and TrimmingSession to type imports
import { User, Crop, PlantBatch, Location, InventoryItem, Formula, FormulaSchedule, Genetics, Notification, Expense, UserRole, Task, Announcement, PlantBatchStatus, LogEntry, MaintenanceLog, MotherPlant, Equipment, TrimmingSession } from '../types';
...
interface AuthContextType {
  ...
  // Fix: Add activeRole and setActiveRole to the context type.
...
// FIX: Add EquipmentContextType
interface EquipmentContextType {
    equipment: Equipment[];
    saveEquipment: (equipment: Equipment) => void;
    deleteEquipment: (equipmentId: string) => void;
}
// FIX: Add TrimmingContextType
...
// FIX: Add EquipmentContext
const EquipmentContext = createContext<EquipmentContextType | undefined>(undefined);
// FIX: Add TrimmingContext
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // --- Data Management Logic ---
    const exportFullBackup = () => {
        try {
            const appState = {
                // FIX: Add equipment and trimmingSessions to backup
                users, allCrops, plantBatches, motherPlants, genetics, locations, tasks, inventory, formulas, formulaSchedule, expenses, notifications, announcements, maintenanceLogs, equipment, trimmingSessions
            };
    ...
    const importFullBackup = (jsonData: string) => {
        try {
            ...
            // FIX: Add equipment and trimmingSessions to backup import
  ...
  const saveCrop = (crop: Crop) => {
        ...
        newLogEntries.forEach(log => {
            if (log.irrigation && !log.irrigation.cost && log.irrigation.volume > 0) {
                ...
                // FIX: Property 'week' does not exist on type 'StageInfo'. Use 'weekInStage' instead.
  ...
  // FIX: Add equipment and trimming session save/delete functions
  const saveEquipment = createSave(setEquipment);
  const deleteEquipment = createDelete(setEquipment);
  const saveTrimmingSession = createSave(setTrimmingSessions);
